...
for i in range(N):#se realiza un ciclo for que va hasta la cant de products
    #se pide los la entrada de los datos de cada product 
    listcodigo.append(int(input()))#por medio de append se añade cada codigo ingresado al conjunto de lista q hay
    listnombre.append(input())#igualmente se añade a lsita los nomres ingresados de tipo string
    listcant.append(float(input()))
    listVunt.append(float(input()))
    listipoiva.append(int(input()))
    totalsiniva.append(listcant[i]*listVunt[i])#se realiza operacion de cada lista en su respectia posicion para saber el subtotal de cada productos ingresados
    ...
    totalfinal=totalfinal+totalconiva[i]#se suma todos los totales de los productos de gaurdados en listas
    ivatotal=ivatotal+iva[i]#e suman todos los ivas para la suma de un total de solo iva de los procductos
